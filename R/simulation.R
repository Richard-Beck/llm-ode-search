...
# ---- main ----
simulate_from_spec <- function(spec,mdl=NULL, seed=1){
  spec <- validate_spec(spec)
  t0 <- spec$sampling$t0; t1 <- spec$sampling$t1; dt <- spec$sampling$dt
  times <- seq(t0, t1, by=dt)
  
  if(is.null(mdl)) mdl <- rxode2::rxode(spec$model$ode_text) ## recompile model if not done already
...
#' Make a model specification "transparent" for detailed simulation
#'
#' This utility function modifies a model specification object to make all
#' internal states directly observable and increases the sampling frequency
#' to generate a high-resolution time series.
#'
#' @param spec A list representing the model specification.
#' @param n_points An integer specifying the total number of time points to sample.
#'
#' @return A modified list representing the updated specification.
#'
#' @examples
#' \dontrun{
#' spec <- jsonlite::fromJSON("data/ground_truth_specs/droop.json")
#' transparent_spec <- make_spec_transparent(spec, n_points = 200)
#' detailed_df <- simulate_from_spec(transparent_spec)
#' plot(detailed_df)
#' }
make_spec_transparent <- function(spec, n_points = 100) {
  
  # 1. Make all states observable
  # --------------------------------------------------------------------------
  # Get the list of all state variables from the model definition.
  all_states <- spec$model$states
  
  # Create a new 'observe' list where each state is mapped to itself.
  # This ensures that the simulation output will include a column for every state.
  spec$observe <- setNames(as.list(all_states), all_states)
  
  
  # 2. Increase sampling frequency
  # --------------------------------------------------------------------------
  # Ensure the sampling block exists and has the required fields.
  if (!is.null(spec$sampling) && !is.null(spec$sampling$t0) && !is.null(spec$sampling$t1)) {
    t0 <- spec$sampling$t0
    t1 <- spec$sampling$t1
    
    # Calculate the new time step 'dt' to achieve the desired number of points.
    # The number of intervals is n_points - 1.
    spec$sampling$dt <- (t1 - t0) / (n_points - 1)
  } else {
    warning("Sampling block (spec$sampling) is missing or incomplete; could not adjust dt.")
  }
  
  # Return the modified specification object.
  return(spec)
}
